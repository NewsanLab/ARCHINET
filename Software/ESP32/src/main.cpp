...
#define BUF_SIZE 256 // Tamaño del buffer para comunicaciones UART 
...
int findEndpoint(const String &ep) {
  for (int i = 0; i < endpointCount; i++) {
    if (endpoints[i].endpoint == ep) return i;
  }
  return -1;
}
//==============Solicitud GET ======================
...
void handleDynamic() {
  ...
  if (path.startsWith("/")) path = path.substring(1); // quito /
