...
def main():
    ...
    print("\n--- TEST: CONNECT ---")
    esp.solicitar_comando({"cmd": "CONNECT", "ssid": "xxxx", "pass": "xxx"})
    ...
    esp.solicitar_comando({
        "cmd": "GET",
        "url": "http://wifitest.adafruit.com/testwifi/index.html" #Consulta de API GET 
    })
    esp.leer_respuesta(timeout=50)
    print("\n--- TEST: GET ---")
    esp.solicitar_comando({
        "cmd": "GET",
        "url": "http://jsonplaceholder.typicode.com/todos/1" #Consulta de API GET 
    })
    esp.leer_respuesta(timeout=50)
